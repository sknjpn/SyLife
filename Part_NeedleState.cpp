...
Part_NeedleState::Part_NeedleState(const std::shared_ptr<PartConfig>& partConfig)
	: PartState(partConfig)
	, m_Part_NeedleAsset(std::dynamic_pointer_cast<Part_NeedleAsset>(partConfig->getPartAsset()))
{}
...
void Part_NeedleState::load(Deserializer<ByteArray>& reader)
{
	reader >> m_heat;
}

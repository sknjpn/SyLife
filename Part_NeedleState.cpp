...
void Part_NeedleState::draw(const CellState &cellState) const {
  ...
  auto t2 =
      Transformer2D(Mat3x2::Scale(1.0, Max(m_heat - 4.0, 0.0) * 1.0 + 1.0));
  ...
  shape.getPreRenderTexture()
      ...
      .draw(shape.getBoundingRect().pos,
            ColorF(1.0, Max(m_heat - 4.0, 0.0) * 0.9 + 0.1));
}
...
void Part_NeedleState::update(CellState &cellState) {
  ...
  if (m_heat < 0) {
    ...
    auto p = cellState.getWorldPosition(
        getPartConfig()->getPosition() +
        Vec2::Up().rotated(getPartConfig()->getRotation()) * 50.0);

    for (auto i : World::GetInstance()->getCellStateKDTree().knnSearch(2, p)) {
      auto &t = World::GetInstance()->getCellStates()[i];

      if (!t->isDestroyed() &&
          t->getRadius() > (t->getPosition() - p).length() &&
          t->m_cellAsset != cellState.m_cellAsset) {
        // armor判定
        if (t->getCellAsset()->getNucleusAsset()->getArmor() <=
            m_Part_NeedleAsset->getPenetrating()) {
          t->destroy();

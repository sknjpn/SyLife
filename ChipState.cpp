...
void ChipState::setWaveVelocity(const Vec2& waveVelocity)
{
...
Vec2 ChipState::getCentroid() const
{
...
void ChipState::sendTo(const shared_ptr<ChipState>& chip, double value)
{
	chip->m_nutrition += value;
	m_nutrition -= value;
}

void ChipState::load(const JSONValue& json)
{
	setWaveVelocity(json[U"waveVelocity"].get<Vec2>());
	m_nutrition = json[U"nutrition"].get<double>();
}

void ChipState::save(JSONWriter& json) const
{

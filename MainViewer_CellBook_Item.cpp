...
void MainViewer::CellBook::Item::update()
{
	...
	if (isMouseover())
	{
		...
		if (MouseL.down())
		{
			...
			if (!getParentViewer()->getParentViewer<MainViewer>()->hasChildViewer(m_cellAsset->getName()))
				getParentViewer()->getParentViewer<MainViewer>()->addChildViewer<CellAssetViewer>(m_cellAsset);
		}
	}
}

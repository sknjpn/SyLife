...
std::shared_ptr<PartState> Part_PoisonerAsset::makePartState(const std::shared_ptr<PartConfig>& partConfig) const
{
	return MakeShared<Part_PoisonerState>(partConfig);
}

void Part_PoisonerAsset::load(const JSONValue& json)
{
	PartAsset::load(json);
...
void Part_PoisonerAsset::save(JSONWriter& json) const
{
	PartAsset::save(json);

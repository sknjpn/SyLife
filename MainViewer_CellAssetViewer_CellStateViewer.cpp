...
MainViewer::CellAssetViewer::CellStateViewer::CellStateViewer(const std::shared_ptr<CellState>& cellState)
...
{
	if (m_cellState->isDestroyed()) { destroy(); return; }
...

	{
		...
		font(U"寿命:", Ceil(m_cellState->getCellAsset()->getLifespanTime() - m_cellState->m_startTimer), U"秒").draw(Vec2::Zero(), Palette::Black);
		moveDrawPos(0, 20);
		...
		moveDrawPos(0, 20);
		{
			...
			font(U"エレメント:", int(m_cellState->m_storage.getElement()), U"elm").draw(Vec2::Zero(), Palette::Black);
			moveDrawPos(0, 20);
		...
		{
			font(U"産卵まで", Ceil(m_cellState->getCellAsset()->getYieldTime() - m_cellState->m_yieldTimer), U"秒").draw(Vec2::Zero(), Palette::Black);
			moveDrawPos(0, 20);
		}
		else
		{
...

		{
			...
			{
				int num = int(Max(0.0, m_cellState->getCellAsset()->getMaterial().getElement() - m_cellState->m_storage.getElement()));
			...
			for (const auto& protein : m_cellState->getCellAsset()->getMaterial().getProteinList())
			{
				int num = Max(0, protein.second - m_cellState->m_storage.numProtein(protein.first));

				if (num > 0)
				{
					font(protein.first->getNameJP() + U": " + ToString(num) + U"個").draw(Vec2::Zero(), Palette::Red);
					moveDrawPos(0, 20);
				}
			}

...
Part_SynthesizerState::Part_SynthesizerState(
    ...
    : PartState(partConfig),
      m_Part_SynthesizerAsset(std::dynamic_pointer_cast<Part_SynthesizerAsset>(
...
void Part_SynthesizerState::draw(const CellState &cellState) const {
  ...
  const double alpha = Math::Lerp(
      0.1, 0.5, Min(1.0, m_timer / m_Part_SynthesizerAsset->getProductTime()));
...
void Part_SynthesizerState::update(CellState &cellState) {
  ...
  if (m_timer > m_Part_SynthesizerAsset->getProductTime() &&
      cellState.m_storage >=
          m_Part_SynthesizerAsset->getExport()->getMaterial() &&
      cellState.m_cellAsset->getMaxStorage().numProtein(
          m_Part_SynthesizerAsset->getExport()) >
          cellState.m_storage.numProtein(
              m_Part_SynthesizerAsset->getExport())) {
    m_timer = 0.0;

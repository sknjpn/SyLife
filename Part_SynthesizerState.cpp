...
Part_SynthesizerState::Part_SynthesizerState(const std::shared_ptr<PartConfig>& partConfig)
...
void Part_SynthesizerState::update(CellState& cellState)
{
	...
	if (m_timer > m_Part_SynthesizerAsset->getProductTime() &&
		...
		cellState.m_cellAsset->getMaxStorage().numProtein(m_Part_SynthesizerAsset->getExport()) > cellState.m_storage.numProtein(m_Part_SynthesizerAsset->getExport()))
	{
		m_timer = 0.0;
...
void Part_SynthesizerState::load(Deserializer<ByteArray>& reader)
{
	reader >> m_timer;
}
